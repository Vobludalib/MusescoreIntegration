...
MuseScore {
    ...
    FileDialog { // Save Abc as...
        ...
        Button { // Import Abc into Xml
            ...
            { // Remove "file://" from paths and third "/" from  paths in Windows
                path = path.replace(/^(file:\/{2})/, "");
                if (Qt.platform.os == "windows") path = path.replace(/^\//, "");
                return path;
            }

            function prepConvPath(path)
            { // Clean and prepare paths for running converters
            ...
            { // Search ini entries for converters
